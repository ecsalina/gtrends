...
# 	report = gtrends._downloadReport(username, password, ["banana", "pie"], startDt, 2,
# 				1, '2m', '', '', '', '')
# 	assert len(report) == 2

# def testDownloadThreeTerms():
# 	report = gtrends._downloadReport(username, password, ["banana", "pie", "mango"], startDt, 3,
# 				1, '2m', '', '', '', '')
# 	assert len(report) == 3
...
def testDownloadTwoTermCorr():
	report = gtrends._downloadReport(username, password, ["banana", "albero"], startDt, 2,
				1, '2m', '', '', '', '')
	lines = report[0].split("\n")
	assert "banana" in lines[0] and "albero" in lines[0]

def testDownloadFiveTermCorr():
	report = gtrends._downloadReport(username, password, ["banana", "albero", "finestra", "tutelare", "ambiente"], startDt, 2,
				1, '2m', '', '', '', '')
	lines = report[0].split("\n")
